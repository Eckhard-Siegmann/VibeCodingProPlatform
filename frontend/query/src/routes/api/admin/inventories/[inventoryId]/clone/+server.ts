...
import { cloneInventory } from '$lib/server/repositories/inventories';

/**
 * POST /api/admin/inventories/[inventoryId]/clone
...
export const POST: RequestHandler = async ({ params, request, cookies }) => {
	try {
		...
		if (!result.success) {
			throw error(409, result.error ?? 'Failed to clone inventory');
		}
