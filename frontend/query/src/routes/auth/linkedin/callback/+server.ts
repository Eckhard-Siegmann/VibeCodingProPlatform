/**
 * LinkedIn OAuth callback — exchanges code for token, resolves account.
 * Spec: Ch.18.4 | ADR 004 (7-step flow, account linking)
 */

import { redirect } from '@sveltejs/kit';
import type { RequestHandler } from './$types';
import { validateState, getLinkedInProfile, setPendingOAuth } from '$lib/server/oauth';
import { findUserByLinkedInId, findUserByEmail, linkOAuthProvider } from '$lib/server/repositories/auth';
...
export const GET: RequestHandler = async ({ url, cookies, request }) => {
	...
	try {
		...
		profile = await getLinkedInProfile(code, redirectUri);
	} catch (err) {
	...
	// Step 4a: Check if user already linked via linkedin_id
	const existingByProvider = findUserByLinkedInId(profile.providerUserId);
	...
	if (existingByEmail) {
		linkOAuthProvider(existingByEmail.user_id, 'linkedin', profile.providerUserId);
